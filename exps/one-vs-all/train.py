...
class DiagnosticsBrain(sb.Brain):
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        """
        _, lens = batch.signal
        lab, _ = batch.cough_label_encoded
...
def dataio_prep(hparams):
    ...
    @sb.utils.data_pipeline.takes("cough-label")
    @sb.utils.data_pipeline.provides("cough_label_encoded")
    def label_pipeline(label):
        """Defines the pipeline to process the input label."""
        label_encoder.lab2ind = {'non':0, 'cough':1}
    ...
    for dataset in data_info:
        datasets[dataset] = sb.dataio.dataset.DynamicItemDataset.from_json(
            ...
            output_keys=["id", "signal", "duration", "file_path", "cough_label_encoded", "symptom"],
        )
